...
Surfaces the data produced by ``pipelines.import_10yr_kcc`` (Rusyn et al. 2024,
...
from db.models import Agent, IarcMonographKcCall, IarcMonographKcStrength
...
def list_volumes(db: Session = Depends(get_db)) -> list[dict]:
    ...
    return [
        {"volume": v, "year": y}
        for v, y in sorted(pairs, key=lambda p: int(p[0]) if str(p[0]).isdigit() else 0)
    ]
...
def agent_matrix(agent_id: str, db: Session = Depends(get_db)) -> dict:
    ...
    calls = db.scalars(
        select(IarcMonographKcCall).where(IarcMonographKcCall.agent_id == agent_id)
    ).all()
    if not calls:
        return {
            "agent_id": agent_id,
    ...
    strengths = db.scalars(
        select(IarcMonographKcStrength).where(IarcMonographKcStrength.agent_id == agent_id)
    ).all()
    strength_out = {
