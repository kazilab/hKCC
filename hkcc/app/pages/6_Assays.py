...
# "mapped to one or more KCCs" was not true of the whole catalogue: a handful of
# assays attach to Layer 2 candidate domains instead, so they carry no KCC link
# and disappear from the KCC filter. Stated rather than left as a silent gap —
# they are not unmapped, they are mapped to the other layer.
_domain_only = [a for a in all_assays if not a.get("kcc_ids")]
_caption = f"Standard wet-lab and high-throughput readouts mapped to one or more KCCs. {breakdown}"
if _domain_only:
    _caption += (
        f" · {len(_domain_only)} attach to Layer 2 candidate domains rather than a KCC, "
        "so they appear only under “All KCCs”."
    )
st.caption(_caption)
...
if source_filter != "all":
    filtered = [a for a in filtered if a.get("source", "unknown") == source_filter]
if design_filter != "all":
    filtered = [a for a in filtered if any(sd.get("design") == design_filter for sd in a.get("study_designs") or [])]
...
for i, a in enumerate(page_rows):
    with cols[i % 2]:
        with st.container(border=True):
            ...
            # Paper-authoritative subgroup taxonomy per KCC (Supplementary Tables 4/5).
