...
# *tables* but never ALTERs an existing one, so an older ``hkcc.db`` read by newer
# model code would lack these. There is no migration step in this project, so this
# is the only place that heals such drift.
...
_SQLITE_LATE_COLUMNS: dict[str, list[tuple[str, str]]] = {
    ...
    "evidence": [
        ...
        ("source_track", "VARCHAR(16) NOT NULL DEFAULT '10yr-iarc'"),
        ("source_count", "SMALLINT"),
        ("data_role", "VARCHAR(16)"),
    ],
    "references": [
...
def ensure_sqlite_schema(bind=engine) -> None:
    ...
    try:
        ...
        # ALTER TABLE cannot add a CHECK constraint in SQLite, so constraints
        # introduced after a table's first release never reached shipped files.
        # This rebuilds such a table with the ORM's own DDL.
        from hkcc.db.schema_repair import ensure_constraints

        ensure_constraints(bind)
