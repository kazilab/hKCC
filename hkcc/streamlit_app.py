...
def _ensure_schema_once() -> bool:
    ...
    A database file that lags the code (missing a late column/table) would crash
    on first query. Best-effort and SQLite-only; see
    :func:`db.session.ensure_sqlite_schema`.
    """
    try:
        from hkcc.db.session import ensure_sqlite_schema
...
_page_defs = [
    ...
    ("9a_Methodology.py", dict(title="Methodology", icon=":material/science:")),
    ("9b_IARC_Matrix.py", dict(title="IARC matrix", icon=":material/dataset:")),
]
