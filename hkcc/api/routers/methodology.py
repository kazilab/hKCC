"""Methodology endpoints — KCAD glossary, column dictionary, source paper, scoring rules."""
...
from hkcc.db.config import KCAD_PAPER_REF_ID
from hkcc.db.models import KcadAbbreviation, KcadColumnDefinition, Reference
from hkcc.db.session import get_db

router = APIRouter(prefix="/methodology", tags=["methodology"])


@router.get("/evidence-rules", response_model=dict)
def evidence_rules(db: Session = Depends(get_db)) -> dict:
    """How every ``evidence.score`` was derived, with live row counts.

    The scoring rules are the scientific product of this database, so they are
    served rather than left to a Markdown file that does not ship in the wheel.
    """
    return rules.evidence_rules_payload(db)
