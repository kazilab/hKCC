...
def render_live_feeds(*, show_header: bool = True) -> None:
    ...
    with tab_pubchem:
        ...
        if st.button("Search PubChem", key="lf_pc_go"):
            with st.spinner("Calling PubChem…"):
                try:
                    cids = pubchem.search_compound_by_name(name)
    ...
    with tab_screening:
        ...
        if st.button("Load assay summary", key="lf_assay_go"):
            with st.spinner("Loading assay summary…"):
                try:
                    _, rows = pubchem.assay_summary_table(int(cid_in), max_rows=nrows)
                except Exception as e:
                    st.error(str(e))
                    rows = []
            if rows:
    ...
    with tab_openalex:
        ...
        if st.button("Search OpenAlex", key="lf_oa_go"):
            with st.spinner("Calling OpenAlex…"):
                try:
                    works = openalex.search_works(q, mailto=mailto, per_page=n)
                except Exception as e:
                    st.error(str(e))
                    works = []
            if works:
    ...
    with tab_epa:
        ...
        if st.button("Try CCTE keyword search", key="lf_ccte_go"):
            res = ccte.try_chemical_keyword_search(cq, use_key or None)
            if res.get("skipped"):
                st.info(res["message"])
            elif res.get("ok"):
