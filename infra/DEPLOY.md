...
also load the demo seed inside the container, set `HKCC_RUN_SEED=1` for the
first run, then remove it:
...
4. Deploy Streamlit app; set secrets (see `.streamlit/secrets.toml.example`):
   ...
   **Without secrets**, the app runs in **demo mode** using bundled `data.js`
   (no Postgres needed). This is the same demo experience you see in the
   public Streamlit Cloud sandbox.
...
5. Tag releases:
   ...
   Then upload `exports/0.1.0/` to Zenodo (manual until DOI automation lands).
...
| `HKCC_RELEASE_TAG` | Version stamp used in citations, exports, `/health` | `dev` |
...
| `HKCC_RUN_SEED` | API entrypoint re-runs the demo seed | `0` |
| `OPENALEX_MAILTO` | OpenAlex polite-pool email | `hkcc@example.org` |
